...
| **13. Missing image amounts** | Blank raw amount remains `None`; `EvidenceProcessor` follows `event_id -> images.related_event_id -> image` and accepts local OCR facts | `test_evidence.test_missing_amount_uses_event_to_image_link_and_keeps_provenance`, audit `missing_event_amount_remains_unknown` | Core pipeline yes; local OCR optional | Image-only amounts fail closed when local OCR is unavailable or ambiguous; they are never treated as zero. |
| **14. Messages/images are untrusted data** | `evidence` bounded fact types and provenance checks; reconciliation never executes content | `test_evidence`, `test_security`, audit `hostile_instruction_is_not_fact` | Yes | Covered by local extraction rules. |
...
| Do not invent financial records/options | Evidence validation requires existing same-user linked event; plans originate in supplied options; reconciliation edits existing event only | `test_security`, `test_plans` | Yes | Covered. `income_confirmed` is only attached to existing evidence context, not a new ledger record. |
...
| **24. Explanation consistency** | `build_explanation_facts`, deterministic template, `validate_explanation` | `test_explanations` | Yes | Covered. The template can only state selected structured facts. |
